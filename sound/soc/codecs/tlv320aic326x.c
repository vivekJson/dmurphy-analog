...
static int aic3262_codec_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_dapm_new_controls(codec, aic3262_dapm_widgets,
...
static int __init aic3262_init(void)
{
	return platform_driver_register(&aic326x_codec_driver);
}
module_init(aic3262_init);

static void __exit aic3262_exit(void)
{
	platform_driver_unregister(&aic326x_codec_driver);
}
module_exit(aic3262_exit);
