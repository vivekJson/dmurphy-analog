			    ...
			    struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
			...
			snd_soc_write(codec, mute_reg, 0x80);
			snd_soc_update_bits(codec, AIC3262_HP_CTL,
	...
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_update_bits(codec, AIC3262_HP_CTL,
				AIC3262_HP_STAGE_MASK ,
				AIC3262_HP_STAGE_25 << AIC3262_HP_STAGE_SHIFT);
		hpl_hpr = snd_soc_read(codec, AIC3262_HP_AMP_CNTL_R1);
		if((hpl_hpr & 0x3) == 0x3) {
			snd_soc_update_bits(codec, AIC3262_HP_AMP_CNTL_R1,
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		snd_soc_write(codec, mute_reg, 0xb9);
		snd_soc_write(codec, AIC3262_POWER_CONF,
				snd_soc_read(codec, AIC3262_POWER_CONF));
		break;
	default:
		BUG();
		return -EINVAL;
			     ...
			     struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		sync_needed = aic3xxx_reg_read(codec->control_data,
							AIC3262_DAC_PRB);
...
int aic3262_ops_adaptivebuffer_swap(struct snd_soc_codec *codec, int mask)
{
	...
	for (i = 0; i < sizeof(sbuf)/sizeof(sbuf[0]); ++i) {
		if ((mask & sbuf[i][0])) {
			aic3xxx_set_bits(codec->control_data, sbuf[i][1],
					0x1, 0x1);
				  ...
				  enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_OFF:
		...
		/* force all power off */
		if (codec->component.dapm.bias_level == SND_SOC_BIAS_STANDBY) {
			snd_soc_update_bits(codec, AIC3262_REF_PWR_DLY,
...
static int aic3262_codec_probe(struct snd_soc_codec *codec)
{
	...
	mutex_init(&codec->component.io_mutex);
